...
async def get_current_user(request: Request):
    ...
    if not id_token:
        raise HTTPException(status_code=303, detail="Unauthorized access. Redirecting to login.", headers={"Location": "/?error=Please log in first"})

    try:
        user = google.oauth2.id_token.verify_firebase_token(id_token, firebase_request_adapter)
        if not user:
            raise HTTPException(status_code=303, detail="Invalid session. Redirecting to login.", headers={"Location": "/?error=Invalid session, please log in again"})
        return user  # User is authenticated
    except ValueError:
        raise HTTPException(status_code=303, detail="Authentication error. Redirecting to login.", headers={"Location": "/?error=Authentication error, please log in again"})
...
async def add_driver_page(request: Request, user: dict = Depends(get_current_user)):
    ...
    return templates.TemplateResponse("add_driver.html", {"request": request, "success_message": success_message})
